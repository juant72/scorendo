...
import { Loader2, Trophy, Calendar, Users, ChevronRight, ArrowLeft, Shield, Sparkles } from 'lucide-react';
                ...
                {matchdays.length > 0 ? matchdays.map((phase: any) => (
                  <div key={phase.id} className="glass-strong rounded-[2.5rem] p-10 border-white/10 group hover:border-primary/40 transition-all shadow-2xl relative overflow-hidden">
                     ...
                     <div className="mt-12 pt-10 border-t border-white/5 flex flex-wrap gap-8">
                        {phase.matches?.slice(0, 4).map((m: any) => (
                           <div key={m.id} className="flex items-center gap-3 glass py-2 px-4 rounded-full border-white/5 opacity-40 group-hover:opacity-100 transition-all">
                              <span className="text-[10px] font-black text-white">{m.homeTeam.code}</span>
                              <span className="text-[8px] font-black text-primary">VS</span>
                              <span className="text-[10px] font-black text-white">{m.awayTeam.code}</span>
                           </div>
                        ))}
                        {phase.matches?.length > 4 && (
                           <div className="flex items-center text-[10px] font-black text-muted-foreground uppercase tracking-widest pl-2">
                              + {phase.matches.length - 4} Matches
                           </div>
                        )}
                     </div>
                  </div>
