      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-12 lg:py-16">
        ...
        <div className="relative rounded-[3.5rem] overflow-hidden stadium-shadow mb-20 group border border-white/10">
          ...
          <img 
            ...
            alt="Stadium Arena" 
            className="w-full h-[450px] object-cover scale-105 group-hover:scale-100 transition-transform duration-1000" 
          />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8">
             <div className="flex items-center justify-between gap-6 mb-8">
                <h2 className="text-xs font-black uppercase tracking-[0.4em] text-primary/60 flex items-center gap-4">
                   Tournament Hub
                </h2>
                <div className="flex bg-midnight/50 p-1 rounded-2xl border border-white/5 backdrop-blur-xl">
                   <button 
                     onClick={() => setActiveTab('matches')}
                     className={`flex items-center gap-2 px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'matches' ? 'bg-primary text-midnight shadow-lg' : 'text-muted-foreground hover:text-white'}`}
                   >
                      <LayoutGrid className="w-3 h-3" /> Matches
                   </button>
                   <button 
                     onClick={() => setActiveTab('rankings')}
                     className={`flex items-center gap-2 px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'rankings' ? 'bg-primary text-midnight shadow-lg' : 'text-muted-foreground hover:text-white'}`}
                   >
                      <ListOrdered className="w-3 h-3" /> Ranking
                   </button>
                </div>
             </div>
             ...
             <div className="space-y-6">
                {activeTab === 'rankings' ? (
                  ...
                  <div key={phase.id} className="glass-strong rounded-[2.5rem] p-10 border-white/10 group hover:border-primary/40 transition-all shadow-2xl relative overflow-hidden">
                     <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none" />
                     
                     <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
                        <div className="space-y-2">
                           <div className="text-[10px] font-black text-primary/80 uppercase tracking-[0.3em]">
                              {new Date(phase.startDate).toLocaleDateString(undefined, { month: 'long', day: 'numeric' })}
                           </div>
                           <h4 className="text-4xl font-black text-white uppercase italic tracking-tighter leading-none">{phase.name}</h4>
                        </div>

                         <div className="flex flex-wrap items-center gap-4">
                            {/* Deduplicate buttons logic: only show one per tier */}
