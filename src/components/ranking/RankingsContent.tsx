...
export function RankingsContent({ users, currentUserWallet }: RankingsContentProps) {
  const { locale } = useAuthStore();
  const t = locales[locale].ranking;
    ...
    <div className="max-w-4xl mx-auto px-4 py-12 lg:py-20">
      ...
      <div className="text-center mb-12">
        <h1 className="text-4xl sm:text-5xl font-black mb-4 uppercase italic">
          {t.h1} <span className="text-gradient-gold">{t.h1Highlight}</span>
        </h1>
        <p className="text-muted-foreground max-w-xl mx-auto uppercase tracking-widest text-[10px] font-black">
          {t.subtitle}
        </p>
      </div>
      ...
      <div className="flex items-end justify-center gap-2 sm:gap-6 mb-16 h-48 px-2">
        {users[1] && <Podium user={users[1]} rank={2} height="h-28" color="bg-zinc-300" tPts={t.pts} />}
        {users[0] && <Podium user={users[0]} rank={1} height="h-36" color="bg-gold" tPts={t.pts} />}
        {users[2] && <Podium user={users[2]} rank={3} height="h-24" color="bg-amber-700" tPts={t.pts} />}
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-white/5 text-xs uppercase text-muted-foreground border-b border-white/5">
              <tr>
                <th className="px-6 py-4 font-bold tracking-wider">{t.table.rank}</th>
                <th className="px-6 py-4 font-bold tracking-wider">{t.table.predictor}</th>
                <th className="px-6 py-4 font-bold tracking-wider text-right">{t.table.points}</th>
                <th className="px-6 py-4 font-bold tracking-wider text-right hidden sm:table-cell">{t.table.correct}</th>
                <th className="px-6 py-4 font-bold tracking-wider text-right hidden md:table-cell">{t.table.accuracy}</th>
              </tr>
            </thead>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        <div className={`font-semibold transition-colors ${isME ? 'text-primary' : 'text-foreground group-hover:text-primary'}`}>
                          {displayName} {isME && <span className="text-[8px] bg-primary text-black px-1.5 py-0.5 rounded ml-1 font-black uppercase">{t.you}</span>}
                        </div>
...
function Podium({ user, rank, height, color, tPts }: { user: any, rank: number, height: string, color: string, tPts: string }) {
  const shortAddress = `${user.walletAddress.slice(0, 4)}...${user.walletAddress.slice(-4)}`;
  const displayName = user.displayName || shortAddress;
  ...
  return (
    <div className="flex flex-col items-center justify-end w-24 sm:w-32">
      <div className="mb-3 text-center">
        <div className="font-bold text-sm text-foreground truncate w-full px-2 text-white" title={displayName}>
          {displayName}
        </div>
        <div className="text-xs font-black text-primary">{user.totalPoints} {tPts}</div>
      </div>
